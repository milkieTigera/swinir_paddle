...
def drop_path(x, drop_prob: float = 0., training: bool = False, scale_by_keep: bool = True):
    ...
    random_tensor = paddle.bernoulli(paddle.empty(shape))
...
class WindowAttention(nn.Layer):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        init.constant_(self.relative_position_bias_table)
...
class SwinIR(nn.Layer):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=3,
        ...
        if self.ape:
            ...
            init.constant_(self.absolute_pos_embed, 0.0)
    ...
    def forward(self, x):
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            x = self.conv_after_body(self.forward_features(x)) + x
            x = self.conv_before_upsample(x)
            x2 = self.lrelu(self.conv_up1(F.interpolate(x, scale_factor=2, mode='nearest')))
            if self.upscale == 4:
                x4 = self.lrelu(self.conv_up2(F.interpolate(x2, scale_factor=2, mode='nearest')))
            x4 = self.conv_last(self.lrelu(self.conv_hr(x4)))
            x2 = self.conv_last(self.lrelu(self.conv_hr(x2)))
        else:
            # for image denoising and JPEG compression artifact reduction
        ...
        x4 = x4 / self.img_range + self.mean
        x2 = x2 / self.img_range + self.mean

        return x2[:, :, :H * (self.upscale//2), :W * (self.upscale//2)], x4[:, :, :H * self.upscale, :W * self.upscale]
...
if __name__ == '__main__':
    ...
    model = SwinIR(upscale=4, in_chans=3, img_size=(64,64), window_size=8,
                    img_range=1., depths=[6, 6, 6, 6, 6, 6], embed_dim=180, num_heads=[6, 6, 6, 6, 6, 6],
                    mlp_ratio=2, upsampler='pixelshuffle', resi_connection='1conv')
    #     print(model)
    path ='/home/aistudio/swinIR_paddle.pdparams'
