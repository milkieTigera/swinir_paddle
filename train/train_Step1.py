...
from utils import  LossStep1, evaluation
import os
...
train_loader = paddle.io.DataLoader(train_set, batch_size=3, num_workers=0, shuffle=True, return_list = True )
...
base_lr = 1*1e-4   
max_score = 0
scheduler = paddle.optimizer.lr.MultiStepDecay(base_lr, milestones=[ 100000, 150000], gamma=0.5)
optimizer = paddle.optimizer.Adam(learning_rate=scheduler, parameters=model.parameters(), beta1=0.9, beta2 =0.999)

# resume_path ='/home/aistudio/work/output/model_temp.param'
resume_path =''
start_iter = 0
...
while iter1<num_iters:
    for i, train_data in enumerate(train_loader):
            ...
            if iter1% 100==0:
                print(f"epoch {(iter1 - 1) // (iters_per_epoch + 1)} {iter1}/{num_iters} total_loss {total_loss.item()} MSE_loss_x2: {MSE_loss_x2.item()} MS_MSSIM_Loss_x2: {MS_SSIM_loss_x2.item()} MSE_loss_x4: {MSE_loss_x4.item()} MS_MSSIM_Loss_x4: {MS_SSIM_loss_x4.item()} ")
            scheduler.step()
            ...
            if iter1%3000==0:
                ...
                if score>max_score:
                    paddle.save(model.state_dict(),model_path+'/model_best1_Step1.param')
                    max_score= score
